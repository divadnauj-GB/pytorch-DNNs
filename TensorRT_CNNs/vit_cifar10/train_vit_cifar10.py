...
parser.add_argument('--dimhead', default="192", type=int)
...
testloader = torch.utils.data.DataLoader(testset, batch_size=args.eval_batch_size, shuffle=False, num_workers=8)

classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')

if args.net=="vit":
    # ViT for cifar10
    net = models.VisionTransformer(
        image_size = size,
        patch_size = args.patch,
        num_layers=6,
        num_heads = 8,        
        hidden_dim= int(args.dimhead),
        mlp_dim= 4*int(args.dimhead),
        dropout=0.1,
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.n_epochs, eta_min=args.min_lr)
